...
\alpha_0(a,s)
:= d_{\bar r_0,k_0}^{\star,\gamma'}(a,s)
+\tau^{\star-1}\widetilde d_{\bar r_0,k_0}^{\star,\gamma'}(a,s)
-\frac{\nu(a\mid s)}{\pi_0(a\mid s)}
\left\{\rho_{\bar r_0,k_0}^{\star,\gamma'}(s)
+\tau^{\star-1}\widetilde\rho_{\bar r_0,k_0}^{\star,\gamma'}(s)\right\}.
\]
\end{theorem}
...
&\quad
  ...
  \rho_n^{\pi,\gamma}(S_i)
  \left\{
  \frac{\pi(A_i\mid S_i)}{\pi_n(A_i\mid S_i)}-1
  \right\}.
\end{align*}
...
\]
and let \(d_n^\star\) and \(\widetilde d_n^\star\) estimate the ordinary and
total advantage-weighted state--action representers. Define
\[
\rho_n^\star(s):=\sum_{a\in\mathcal A}\pi_n(a\mid s)d_n^\star(a,s),
\qquad
\widetilde\rho_n^\star(s)
:=\sum_{a\in\mathcal A}\pi_n(a\mid s)\widetilde d_n^\star(a,s).
\]
The resulting estimator of \(\psi_0=E_0\{V_{r_0,k_0}^\star(S)\}\) is
\begin{align*}
\psi_n
&:=
...
\left\{
 d_n^\star(A_i,S_i)
 +\tfrac{1}{\tau^\star}\widetilde d_n^\star(A_i,S_i)
 -\rho_n^\star(S_i)
...
\begin{enumerate}[label=\textbf{(E\arabic**)}, ref=E\arabic**, resume = efficiency1b]
    ...
    \[
      \|d_n^\star\|_{L^\infty(\lambda)}
      + \|\widetilde d_n^\star\|_{L^\infty(\lambda)}
    ...
    The nuisance estimators $d_n^\star$, $\widetilde d_n^\star$, $q_n^\star$, $v_n^\star$, and $r_n$
    ...
    \[
      \|d_n^\star - d_{r_0,k_0}^\star\|_{\mathcal H,r_0}
      + \|\widetilde d_n^\star - \widetilde d_{r_0,k_0}^\star\|_{\mathcal H,r_0}
...
\section{Simulation implementation and diagnostics}
\label{app::normsim}

\paragraph{Neural FORE.}
Actions are represented by four-dimensional one-hot vectors. Each neural FORE
fit uses the variational log-partition objective, minibatches of 512, five
gradient steps per fitted iteration, gradient-norm clipping at 10, and a
log-ratio cap of 10. Four common target-action draws are reused across candidates
within a selection event. The pilot library crosses hidden-layer widths
$(64,64)$ and $(128,128)$, learning rates $3\times10^{-4}$ and $10^{-3}$,
weight decays $10^{-3}$ and $10^{-2}$, and iteration budgets 10, 30, and 100.
Ten pilot seeds yield 160 A-PBV selection events across examples, endpoint
sample sizes, folds, and signed components. The pilot freezes one architecture
and optimizer configuration by selection frequency across these events; ties are broken by
mean within-cell rank, smaller width, and stronger regularization. During the
confirmatory run the stopping library is expanded to 30, 100, and 300 iterations
after the pilot showed that 100 iterations frequently remained on a decreasing
validation path. Neural ratio optimization uses a deterministic maximum of
20,000 rows from each outer training fold. This budget affects only FORE;
logging-policy and Bellman fitting use the complete outer training fold and the
EIF is evaluated on every untouched outer-fold observation. In a four-seed
$n=100000$ audit, capped FORE differed from the exact-ratio estimate by RMSE
0.0014 and at most 0.0022, retained zero clipping and failures, and reduced
per-component ratio-fit time by about a factor of 3.4 relative to full-data
FORE. Setting the cap to zero recovers the full-data appendix diagnostic.
For Examples~1a--1b, the existing fine-grid Bellman evaluation
is held fixed as a high-accuracy nuisance benchmark so that this study isolates
estimated-policy and estimated-ratio error; neither its transition kernel nor
any grid truth enters FORE or A-PBV.
The data-fusion study estimates its transition and Bellman nuisances from the
observed transition source instead.

\paragraph{Main and stress designs.}
The primary logging law is a state-dependent quadratic multinomial logit. The
quadratic sieve is correctly specified, action probabilities remain bounded
away from zero, and the action omitted from the adaptive target has about 5\%
marginal probability. The ordinary-ratio 99th percentile is approximately 12.4
and the ESS fraction approximately 0.93 in the numerical design check. The
appendix stress design uses the original soft-MDP logging law; its corresponding
values are approximately 24.2 and 0.71. We use a $41\times41$ numerical grid.
Relative to a $61\times61$ reference, this changes the adaptive estimand by
0.0017, the target policy by less than $6\times10^{-4}$ uniformly, and the
stationary-weighted ordinary and signed ratios by about 0.2\% and 1.1\% in
relative $L^2$ error.

\paragraph{Held-out A-PBV score.}
Let $w_i$ and $w_j$ be two positive normalized ratio candidates and define
$\ell_{ij}=\log(w_i/w_j)$. Using held-out empirical transition and
initial-source terms, candidate $i$ is compared with candidate $j$ using
\[
\widehat S_{ij}
=\mathbb P_n\!\left[w_i(X)
  \{\ell_{ij}(X)-\gamma(\pi\ell_{ij})(S')\}\right]
 -(1-\gamma)\mathbb P_{0,n}\ell_{ij}(X_0)
 -\frac{1-\gamma}{4}\mathbb P_n
  \{\sqrt{w_i(X)}-\sqrt{w_j(X)}\}^2.
\]
The selected index is the first element of
$\arg\min_i\max_j\widehat S_{ij}$. Because the action space has size four,
$(\pi\ell_{ij})(S')$ and the initial-state analogue are evaluated by exact
action summation. Candidate fitting and validation use a deterministic
60/20/20 split within each outer training fold: the middle block selects the
ordinary ratio and the last block selects the signed components. Within each
20\% validation block, the same held-out rows estimate the transition and
initial-source terms of the empirical A-PBV objective; this improves validation
precision without touching the outer evaluation fold. Simulation truth does not
enter either selection.

FORE returns the normalized discounted state--action ratio $w$. The EIF in the
main paper uses the unnormalized convention, so the implementation sets
$\widehat d=w/(1-\gamma)$ and recovers the state ratio by
\[
\widehat\rho(s)=\sum_a\widehat\pi_0(a\mid s)\widehat d(s,a).
\]
This permits the numerically stable correction
$\widehat d(S,A)\widehat\varepsilon(S,A)+\widehat d(S,A)-
\widehat\rho(S)$ without separately dividing by a fitted behaviour propensity.

\paragraph{Signed advantage-weighted ratio.}
On the ratio-training data define
\[
Z_i=\widehat d^\star(S_i,A_i)
 \{\widehat q^\star(S_i,A_i)-\widehat V^\star(S_i)\},
\qquad
m_\pm=\mathbb P_n(Z_i)_\pm.
\]
We use the successor states weighted by $(Z_i)_+$ and $(Z_i)_-$ as two initial
source measures, fit their normalized FORE ratios $w_+$ and $w_-$ separately,
and set
\[
\widehat{\widetilde d}(x)
=\widehat z(x)
+\frac{\gamma}{1-\gamma}\{m_+w_+(x)-m_-w_-(x)\},
\qquad
\widehat z(s,a):=\widehat d^\star(s,a)
\{\widehat q^\star(s,a)-\widehat V^\star(s)\}.
\]
The Jordan fits estimate only the strictly future component because their
initial measures are supported on successor states. For EIF evaluation we add
the current term \(\widehat z\), evaluate the total ratio at every action, and
set
\[
\widehat{\widetilde\rho}(s)
:=\sum_a\widehat\pi_0(a\mid s)\widehat{\widetilde d}(s,a).
\]
The current state average is zero at the population truth, but it need not be
zero for estimated nuisances and is therefore retained.
A component is set to zero when its empirical source mass is below $10^{-10}$.
Positive and negative components have independent A-PBV selections.

\paragraph{Data fusion.}
The transition source records $(S,A,S')$ and the known logging propensities but
withholds rewards. The auxiliary source records only $(S,Y)$ over the state
domain, with
$E(Y\mid S=s)=g(s)=\sum_a\nu(a\mid s)r_0(s,a)$ for a known uniform randomized
normalization policy $\nu$; actions are absent by design. We fit $g$ once using
$10^6$ outcome-summary observations with noise standard deviation 0.2. A
deterministic 80/20 split selects a small tanh network on quadratic features by
held-out outcome mean-squared error; three source-only refits are averaged and
intercept-calibrated, then frozen across all transition replications. The
selected regression has grid RMSE 0.0024 and induces an estimand change of
0.0030 at target discount 0.8. Within each outer training fold, an
action-specific degree-three ridge sieve estimates the two conditional
transition means under a diagonal Gaussian law, with regularization selected by
held-out next-state mean-squared error and innovation scales estimated on that
validation block. The fitted transition is discretized on the fixed evaluation
grid, after which the normalization and target-policy Bellman equations are
solved deterministically. These selectors receive neither the oracle transition
kernel nor reward or estimand truth. Before the
confirmatory run, the oracle-grid change in the target induced by replacing
$g$ with $\widehat g$ must be smaller than 10\% of the pilot median D-IRL
standard error. This is a labeled readiness diagnostic, not an estimator or
hyperparameter selection rule. The reported confidence intervals condition on
the frozen outcome source and do not include its sampling uncertainty.

\paragraph{Reproducibility and oracle diagnostics.}
Each run stores raw estimates, Monte Carlo summaries, candidate-level A-PBV
rows, the frozen configuration, random seeds, ratio-tail diagnostics, runtime,
and failures. Oracle ratios and fine-grid targets are used only in explicitly
labeled diagnostic output after fitting. In particular, a population-grid
check verifies that the positive-minus-negative FORE normalization above
reconstructs the oracle signed state--action ratio. All estimator comparisons
use common data-generating seeds; changing the ratio estimator never changes the
sampled dataset. The execution order is a two-seed smoke test, the truth-blind
selection pilot, and the locked 300-replication confirmatory study. Example~1a
and data fusion use $n\in\{2500,5000,10000\}$, while Example~1b uses
$n\in\{25000,50000,100000\}$.

\paragraph{Confirmatory fit diagnostics.}
The locked study contains 2,700 successful replications and no ratio failure,
nonfinite inferential quantity, or clipping event. For the ordinary ratio,
A-PBV never selects 30 iterations. The shares selecting 300 iterations are
22\%, 40\%, and 64\% across the three Example~1a sample sizes; 77--79\% in
Example~1b; and 31\%, 59\%, and 79\% in the data-fusion study. We report this
boundary behavior rather than using it to alter the locked library. The
pre-confirmatory exact-ratio audits described above show that the resulting
large-sample FORE estimates are already close to the exact-ratio benchmark.
Across the adaptive-policy cells, the signed positive and negative components
have mean normalized masses 1.000--1.001, effective-sample fractions
0.68--0.73, mean 99th percentiles 14.0--18.7, and no clipping. The strict
artifact assembler verifies all predeclared seeds, fold-level stopping choices,
finite telemetry, frozen-input hashes, and raw-to-summary agreement. Coverage
calibration is computed only after fitting and is never used to select or
suppress a reported cell.
...
\[
z_{r_0,k_0}^\star
:=d_{r_0,k_0}^\star(q_{r_0,k_0}^\star-V_{r_0,k_0}^\star).
\]
By the adjoint definition in \eqref{eqn::tilderho}, for every state--action
function \(h\),
\begin{equation}
\label{eqn::identity2}
E_0\!\left[z_{r_0,k_0}^\star(A,S)
\{(\mathcal T_{r_0,k_0}^\star)^{-1}h\}(A,S)\right]
=E_0\!\left[\widetilde d_{r_0,k_0}^\star(A,S)h(A,S)\right].
\end{equation}
...
\Big] \\
&= \frac{1}{1-\gamma}\,E_{d_{r_0,k_0}^\star}\Big[
\big((I-\Pi_{r_0,k_0}^\star)(\mathcal{T}_{r_0,k_0}^\star)^{-1} r_0\big)(A,S)\;
\big(\Pi_{r_0,k_0}^\star(\mathcal{T}_{r_0,k_0}^\star)^{-1} h\big)(S)
\Big] \\
&= 0,
\end{align*}
where \(E_{d_{r_0,k_0}^\star}\) is expectation under the probability
distribution with density \((1-\gamma)d_{r_0,k_0}^\star\) relative to the
law of \((A,S)\), and the final equality follows from the law of total
expectation, since
\[
...
&= \frac{1}{\tau^\star}\,
E_0\!\left[z_{r_0,k_0}^\star(A,S)\,
\big\{(\mathcal{T}_{r_0,k_0}^\star)^{-1} h\big\}(A,S)\right] \\
&=\frac{1}{\tau^\star}
E_0\!\left[\widetilde d_{r_0,k_0}^\star(A,S)h(A,S)\right],
\end{align*}
where the second equality is \eqref{eqn::identity2}. Hence,
\begin{equation}
...
\begin{equation}
    \label{eqn::alphastar}
    \alpha_0(a,s) := d_{r_0,k_0}^\star(a,s)
    +\frac{1}{\tau^\star}\widetilde d_{r_0,k_0}^\star(a,s).
\end{equation}
...
Applying \eqref{eqn::identity2} with
\(h=\mathcal P_{k_0\beta}\Phi_{r_0,k_0}^\star\) gives
\begin{align*}
&\frac{\gamma}{\tau^\star}
E_0\!\left[z_{r_0,k_0}^\star(A,S)
\{(\mathcal T_{r_0,k_0}^\star)^{-1}
\mathcal P_{k_0\beta}\Phi_{r_0,k_0}^\star\}(A,S)\right] \\
&\qquad=\frac{\gamma}{\tau^\star}
E_0\!\left[\widetilde d_{r_0,k_0}^\star(A,S)
\{\mathcal P_{k_0\beta}\Phi_{r_0,k_0}^\star\}(A,S)\right] \\
&\qquad=\frac{\gamma}{\tau^\star}
E_0\!\left[\widetilde d_{r_0,k_0}^\star(A,S)
\Phi_{r_0,k_0}^\star(S')\beta(S'\mid A,S)\right].
\end{align*}
The last equality follows from the definition of the additive kernel
direction \(k_0\beta\) and Fubini's theorem.
Define 
\[
\begin{aligned}
\beta_{0,1}(s' \mid a, s)
...
+\frac{1}{\tau^\star}\widetilde d_{r_0,k_0}^\star(a,s)
-\rho_{r_0,k_0}^\star(s)
-\frac{1}{\tau^\star}\widetilde\rho_{r_0,k_0}^\star(s).
\end{align*}
Thus, substituting our expression for $\beta_0$, we conclude that
\begin{align*}
   \chi_0(s,a,s') &= d_{r_0,k_0}^\star(a,s)
   +\frac{1}{\tau^\star}\widetilde d_{r_0,k_0}^\star(a,s)
   -\rho_{r_0,k_0}^\star(s)
   -\frac{1}{\tau^\star}\widetilde\rho_{r_0,k_0}^\star(s) \\
...
\tilde\alpha_0(a,s)
=d_{\bar r_0,k_0}^{\star,\gamma'}(a,s)
+\frac{1}{\tau^\star}\widetilde d_{\bar r_0,k_0}^{\star,\gamma'}(a,s)
\]
and
\[
\begin{aligned}
...
-\frac{\nu(a\mid s)}{\pi_0(a\mid s)}
\left\{\rho_{\bar r_0,k_0}^{\star,\gamma'}(s)
+\frac{1}{\tau^\star}\widetilde\rho_{\bar r_0,k_0}^{\star,\gamma'}(s)\right\}.
\]
It follows immediately that
\[
\sum_{\tilde a \in \mathcal{A}} \pi_0(\tilde a\mid s)\,\alpha_0(\tilde a,s) = 0,
\]
by the definitions of the state averages and because
\(\nu(\cdot\mid s)\) sums to one.
...
\[
\pi_{r,v}^\star(a\mid s)
\propto
\mathbf 1\{a\in\mathcal A^\star\}
\exp\!\left[\tau^{\star-1}\{r(a,s)+\gamma v(a,s)\}\right],
\]
and define
\[
\Phi_{r,v}^\star(s)
:=\tau^\star\log\!\sum_{a\in\mathcal A^\star}
\exp\!\left[\tau^{\star-1}\{r(a,s)+\gamma v(a,s)\}\right],
\qquad
V_{q,r,v}^\star(s)
:=\sum_{a\in\mathcal A^\star}\pi_{r,v}^\star(a\mid s)q(a,s).
\]
For arbitrary state--action functions \(d\) and \(\widetilde d\), set
\[
\rho_{d,r}(s):=\sum_{a\in\mathcal A}\pi_r(a\mid s)d(a,s),
\qquad
\widetilde\rho_{\widetilde d,r}(s)
:=\sum_{a\in\mathcal A}\pi_r(a\mid s)\widetilde d(a,s),
\]
where \(\pi_r=\exp(r)\). Throughout this lemma, \(r\) ranges over normalized
log-policy functions, so \(\sum_a\exp\{r(a,s)\}=1\) almost surely.

\begin{lemma}[von Mises expansion for soft optimal value]
\label{lemma::vonmises_softstar}
Let \(q,r,v,d,\widetilde d\in L^\infty(\lambda)\), with \(r\) normalized as
above, and define the
uncentered score
\begin{align*}
\varphi_{r,v,q,d,\widetilde d}(s',a,s)
:={}&V_{q,r,v}^\star(s)
+d(a,s)\{r(a,s)+\gamma V_{q,r,v}^\star(s')-q(a,s)\}\\
&+d(a,s)-\rho_{d,r}(s)\\
&+\frac{1}{\tau^\star}\left[
\widetilde d(a,s)
+\gamma\widetilde d(a,s)\{\Phi_{r,v}^\star(s')-v(a,s)\}
-\widetilde\rho_{\widetilde d,r}(s)
\right].
\end{align*}
Suppose these functions and their true counterparts lie in a common bounded
\(L^\infty(\lambda)\) ball. Then, in a sufficiently small neighborhood of the
truth in \(\|\cdot\|_{\mathcal H,r_0}\),
\begin{align}
\left|E_0\{\varphi_{r,v,q,d,\widetilde d}(S',A,S)\}-\psi_0\right|
\lesssim{}&
\left(
\|d-d_{r_0,k_0}^\star\|_{\mathcal H,r_0}
+\|r-r_0\|_{\mathcal H,r_0}
+\|v-v_{r_0,k_0}^\star\|_{\mathcal H,r_0}
\right)\notag\\
&\quad\times
\left(
\|\mathcal T_{r_0,k_0}^\star(q-q_{r_0,k_0}^\star)\|_{\mathcal H,r_0}
+\|r-r_0\|_{\mathcal H,r_0}
+\|v-v_{r_0,k_0}^\star\|_{\mathcal H,r_0}
\right)\notag\\
&+
\|\widetilde d-\widetilde d_{r_0,k_0}^\star\|_{\mathcal H,r_0}
\left(
\|r-r_0\|_{\mathcal H,r_0}
+\|v-v_{r_0,k_0}^\star\|_{\mathcal H,r_0}
\right).
\label{eq:softstar-vm-total}
\end{align}
The implicit constant depends only on the uniform bounds, \(\gamma\),
\(\tau^\star\), and the local operator bounds used below.
\end{lemma}

\begin{proof}
Abbreviate
\[
q_0:=q_{r_0,k_0}^\star,\quad
v_0:=v_{r_0,k_0}^\star,\quad
d_0:=d_{r_0,k_0}^\star,\quad
\widetilde d_0:=\widetilde d_{r_0,k_0}^\star,
\]
and let \(\pi^\star:=\pi_{r,v}^\star\),
\(\pi_0^\star:=\pi_{r_0,v_0}^\star\),
\(\Delta q:=q-q_0\), and
\(\Delta Q:=(r-r_0)+\gamma(v-v_0)\).
Split the left-hand side of \eqref{eq:softstar-vm-total} into the terms
linear in \(\Delta q\), denoted \(R_q\), and the remaining terms, denoted
\(R_{\mathrm{pol}}\).

For the first part,
\[
R_q
=E_0[V_{\Delta q,r,v}^\star(S)]
+E_0\!\left[d(A,S)
\{\gamma V_{\Delta q,r,v}^\star(S')-\Delta q(A,S)\}\right].
\]
The discounted occupancy identity gives
\[
E_0[V_{\Delta q,r_0,v_0}^\star(S)]
+E_0\!\left[d_0(A,S)
\{\gamma V_{\Delta q,r_0,v_0}^\star(S')-\Delta q(A,S)\}\right]=0.
\]
Subtracting this identity, using Cauchy--Schwarz, and applying the local
Lipschitz bound for the softmax map yields
\[
|R_q|
\lesssim
\left\{
\|d-d_0\|_{\mathcal H,r_0}
+\|r-r_0\|_{\mathcal H,r_0}
+\|v-v_0\|_{\mathcal H,r_0}
\right\}
\|\Delta q\|_{\mathcal H,r_0}.
\]
By Condition~\ref{cond::stationary3} and
Lemma~\ref{lemma::invertibility}, the inverse of
\(\mathcal T_{r_0,k_0}^\star\) is locally bounded, so
\[
\|\Delta q\|_{\mathcal H,r_0}
\lesssim
\|\mathcal T_{r_0,k_0}^\star\Delta q\|_{\mathcal H,r_0}.
\]
This proves the first product in \eqref{eq:softstar-vm-total} that contains the
Bellman error.

It remains to control \(R_{\mathrm{pol}}\). The identity
\[
E_0[d(A,S)\{r-r_0\}(A,S)]
+E_0[d(A,S)-\rho_{d,r}(S)]
=
E_0\!\left[d(A,S)
\{r-r_0+1-\exp(r-r_0)\}(A,S)\right]
\]
uses \(\pi_r/\pi_0=\exp(r-r_0)\). Taylor's theorem therefore bounds this
quantity by \(O(\|r-r_0\|_{\mathcal H,r_0}^2)\), uniformly over bounded
\(d\).

The first-order change of the target softmax satisfies
\[
\pi_{r,v}^\star-\pi_0^\star
=
\frac{\pi_0^\star}{\tau^\star}
\left\{\Delta Q-\Pi_{r_0,k_0}^\star\Delta Q\right\}
+O(\|\Delta Q\|_{\mathcal H,r_0}^2).
\]
Substitution into the plug-in and ordinary Bellman terms, followed by the
discounted occupancy identity, gives the first-order policy contribution
\[
\frac{1}{\tau^\star}
E_0[z_{r_0,k_0}^\star(A,S)\Delta Q(A,S)].
\]
Replacing \(d_0\) by \(d\) in these terms contributes at most
\[
O\!\left(
\|d-d_0\|_{\mathcal H,r_0}\,
\|\Delta Q\|_{\mathcal H,r_0}
\right).
\]
The second-order softmax remainder is
\(O(\|\Delta Q\|_{\mathcal H,r_0}^2)\); this follows from the bounded
second derivative in Lemma~\ref{lemma::bellmansmoothnessfull}.

For the signed augmentation, Taylor expansion of
\(\Phi_{r,v}^\star\) and the same log-policy identity give
\begin{align*}
&\frac{1}{\tau^\star}E_0\!\left[
\widetilde d(A,S)
+\gamma\widetilde d(A,S)\{\Phi_{r,v}^\star(S')-v(A,S)\}
-\widetilde\rho_{\widetilde d,r}(S)\right]\\
&\qquad=
-\frac{1}{\tau^\star}
E_0\!\left[\widetilde d(A,S)
\{\mathcal T_{r_0,k_0}^\star\Delta Q\}(A,S)\right]
+O(\|\Delta Q\|_{\mathcal H,r_0}^2).
\end{align*}
By the defining adjoint identity \eqref{eqn::identity2},
\[
E_0[\widetilde d_0(A,S)
\{\mathcal T_{r_0,k_0}^\star\Delta Q\}(A,S)]
=
E_0[z_{r_0,k_0}^\star(A,S)\Delta Q(A,S)].
\]
The two first-order policy terms thus cancel. Cauchy--Schwarz bounds the
effect of replacing \(\widetilde d_0\) by \(\widetilde d\) by
\[
O\!\left(
\|\widetilde d-\widetilde d_0\|_{\mathcal H,r_0}
\{\|r-r_0\|_{\mathcal H,r_0}+\|v-v_0\|_{\mathcal H,r_0}\}
\right).
\]
Combining the bounds for \(R_q\) and \(R_{\mathrm{pol}}\) proves
\eqref{eq:softstar-vm-total}.
\end{proof}
...
\;+\; \rho_n^{\pi,\gamma}(s)\Big\{\tfrac{\pi(a \mid s)}{\pi_n(a \mid s)}-1\Big\}.
\]
...
&\quad + \frac{\gamma}{\tau^\star}\,
        ...
        \left[
           \tau^\star \log\!\left( 
            ...
            - v_n^\star(a,s)
        \right] \\
&\quad +d_n^\star(a,s)+\frac{1}{\tau^\star}\widetilde d_n^\star(a,s)
      -\rho_n^\star(s)-\frac{1}{\tau^\star}\widetilde\rho_n^\star(s).
\end{align*}
Let $\varphi_0$ denote the true uncentered influence function corresponding to the nuisances 
$(r_0, v_{r_0,k_0}^\star, q_{r_0,k_0}^\star, d_{r_0,k_0}^\star, \widetilde d_{r_0,k_0}^\star)$, so that $P_0 \varphi_0 = \psi_0$ and $P_0\chi_0 = 0$, where $\chi_0$ is the influence function in Theorem~\ref{theorem::EIFsoftmax}. Then, arguing as in the proof of Theorem \ref{theorem::ALex1},
\begin{align*}
...
It remains to control the remainder term $P_0 \varphi_n - \psi_0$.
By construction, $\varphi_n$ equals
$\varphi_{r_n,v_n^\star,q_n^\star,d_n^\star,\widetilde d_n^\star}$ from
Lemma~\ref{lemma::vonmises_softstar}, with
$\rho_n^\star=\rho_{d_n^\star,r_n}$ and
$\widetilde\rho_n^\star=\widetilde\rho_{\widetilde d_n^\star,r_n}$.
Consequently, \eqref{eq:softstar-vm-total} gives
\begin{align*}
|P_0\varphi_n-\psi_0|
\lesssim{}&
\left(
...
\right)\\
&\quad\times\left(
\|\mathcal T_{r_0,k_0}^\star(q_n^\star-q_{r_0,k_0}^\star)\|_{\mathcal H,r_0}
+\|r_n-r_0\|_{\mathcal H,r_0}
+\|v_n^\star-v_{r_0,k_0}^\star\|_{\mathcal H,r_0}
\right)\\
&+\|\widetilde d_n^\star-\widetilde d_{r_0,k_0}^\star\|_{\mathcal H,r_0}
...
\right).
\end{align*}
The first product is $o_p(n^{-1/2})$ by
Assumption~\ref{assump:rates::softstar}(c), and the second is
$o_p(n^{-1/2})$ by Assumption~\ref{assump:rates::softstar}(b).
