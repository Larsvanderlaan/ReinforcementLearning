...
For the rate statements below, we also write
\(\|h\|_{\mathcal H,r_0}:=\{E_0[h(A,S)^2]\}^{1/2}\); this is the
same norm as \(\|h\|_{\mathcal H,0}\), with the subscript emphasizing that
the action law is \(\pi_0=\exp(r_0)\).
...
\]
Let \(\rho_{r_0,k_0}^\star\) and \(d_{r_0,k_0}^\star\) denote the
unnormalized discounted occupancy ratios in \eqref{eqn::occupancy}, with
\(\pi=\pi_{r_0,k_0}^\star\). Define
\[
z_{r_0,k_0}^\star(a,s)
:=d_{r_0,k_0}^\star(a,s)
\{q_{r_0,k_0}^\star(a,s)-V_{r_0,k_0}^\star(s)\}.
\]
The total advantage-weighted state--action representer is
\begin{equation}
\label{eqn::tilderho}
\widetilde d_{r_0,k_0}^\star
:=\big\{(\mathcal T_{r_0,k_0}^\star)^{-1}\big\}^{*}
z_{r_0,k_0}^\star,
\qquad
\widetilde\rho_{r_0,k_0}^\star(s)
:=\sum_{a\in\mathcal A}\pi_0(a\mid s)
\widetilde d_{r_0,k_0}^\star(a,s),
\end{equation}
where the adjoint is taken in
\(L^2\{\rho_0(s)\pi_0(a\mid s)\}\). Equivalently, for every
\(h\) in this space,
\[
E_0\{\widetilde d_{r_0,k_0}^\star(A,S)h(A,S)\}
=E_0\!\left[z_{r_0,k_0}^\star(A,S)
\{(\mathcal T_{r_0,k_0}^\star)^{-1}h\}(A,S)\right].
\]
This definition includes the current contribution
\(z_{r_0,k_0}^\star\); the successor-state occupancy construction used for
estimation represents only its strictly future component.
...
&= d_{r_0,k_0}^\star(a,s)
   ...
   -\rho_{r_0,k_0}^\star(s)
   -\frac{1}{\tau^\star}\widetilde\rho_{r_0,k_0}^\star(s) \\[4pt]
...
total advantage-weighted representer and the Bellman term
\(\tfrac{\gamma}{\tau^\star}\,\widetilde{d}_{r_0,k_0}^\star(a,s)
\{\Phi_{r_0,k_0}^\star(s') - v_{r_0,k_0}^\star(a,s)\}\).
It also centers \(\widetilde d_{r_0,k_0}^\star\) under the behaviour policy.
Both additions arise because the target policy depends on the soft Bellman
...
total advantage-weighted state--action representers. Define their state
averages under the fitted behaviour policy by
\[
...
\(\psi_0=E_0\{V_{r_0,k_0}^\star(S)\}\) is
\begin{align*}
\psi_n
&:=
...
\(Q\)-evaluation under \(\pi_n^\star\). For the signed representer, let
\(z_n=d_n^\star(q_n^\star-V_n^\star)\). A Jordan decomposition of
\(z_n\) permits separate positive FORE fits initialized at successor states.
Those fits estimate the strictly future term, which must be added to \(z_n\)
to obtain the total \(\widetilde d_n^\star\) in the estimator above.
...
\begin{enumerate}[label=(\alph*)]
    ...
    \item \(\|\widetilde d_n^\star - \widetilde d_{r_0,k_0}^\star\|_{\mathcal{H},r_0}\,
    \bigl(\|r_n - r_0\|_{\mathcal{H},r_0} + \|v_n^\star - v_{r_0,k_0}^\star\|_{\mathcal{H},r_0}\bigr)
    = o_p(n^{-1/2})\).
    
    \item \(\bigl(\|d_n^\star - d_{r_0,k_0}^\star\|_{\mathcal{H},r_0}
      + \|r_n - r_0\|_{\mathcal{H},r_0}
      + \|v_n^\star - v_{r_0,k_0}^\star\|_{\mathcal{H},r_0}\bigr)\,
      \bigl\{\bigl\|\mathcal{T}_{r_0,k_0}^\star(q_n^\star - q_{r_0,k_0}^\star)\bigr\|_{\mathcal{H},r_0}
      +\|r_n-r_0\|_{\mathcal H,r_0}
      +\|v_n^\star-v_{r_0,k_0}^\star\|_{\mathcal H,r_0}\bigr\}
      = o_p(n^{-1/2})\).
\end{enumerate}
...
We study the estimators in a continuous-state treatment MDP with
two state variables, four actions, nonlinear rewards, and Gaussian state
transitions. Samples consist of independent stationary one-step transitions.
The primary logging policy is a smooth state-dependent quadratic multinomial
logit with controlled overlap; its probabilities are estimated rather than
supplied to the estimator. We use five-fold cross-fitting and 300 predeclared
Monte Carlo replications per design point. Example~\ref{example::1a} uses
$n\in\{2500,5000,10000\}$, whereas the policy-adaptive
Example~\ref{example::1b} uses $n\in\{25000,50000,100000\}$. A labeled
appendix stress test restores the original soft-optimal logging policy and its
heavier ratio tails. In the first study, a fine grid supplies simulation truth
and a common high-accuracy Bellman-evaluation nuisance, thereby isolating
estimated policy and ratio error. Grid truth is never available to FORE or
A-PBV. The data-fusion study instead estimates a smooth Gaussian transition
model from the observed transition source and solves fitted-grid Bellman equations.

\paragraph{Estimated-ratio inference.}
For Examples~\ref{example::1a} and~\ref{example::1b}, discounted occupancy
ratios are learned from the same transition sample by neural fitted
occupancy-ratio evaluation (FORE). The implementation uses two-hidden-layer
networks, minibatches of 512 transitions, five stochastic-gradient updates per
outer iteration, and gradient and log-ratio caps of 10. To keep the largest
design practical, neural ratio optimization uses a deterministic maximum of
20,000 outer-training rows per fold; logging-policy fitting and held-out
inference continue to use all assigned rows.
Hyperparameters are frozen in a separate pilot, and the number of fitted-ratio
iterations in $\{30,100,300\}$ is selected within each training fold by held-out adversarial pairwise
Bellman validation (A-PBV). This criterion compares pairwise log ratios through
observable occupancy-balance moments and therefore uses neither the oracle
ratio nor the target estimand. For Example~\ref{example::1b}, the
advantage-weighted ratio is signed. We fit its positive and negative source
measures separately by FORE and subtract the fitted components. The ordinary
ratio and the two signed components are then refit on the full training fold
before evaluating the untouched fold.

Table~\ref{tab:fore-inference} compares the plug-in and D-IRL estimators by
bias, Monte Carlo standard deviation, and RMSE across the 300 locked
replications. For D-IRL we additionally report the mean estimated standard
error, 95\% Wald coverage, and mean interval width. Oracle-ratio evaluations
are retained only as labeled appendix diagnostics.

\IfFileExists{../../experiments/journal_debiased_irl/artifacts/paper_confirmatory_v2_tables/main_inference_table.tex}{%
\input{../../experiments/journal_debiased_irl/artifacts/paper_confirmatory_v2_tables/main_inference_table.tex}%
}{}

All 1,800 estimated-ratio replications completed without a nonfinite ratio fit
or any log-ratio clipping. In Example~\ref{example::1a}, D-IRL coverage is
93.3--96.3\%; in the more variable policy-adaptive
Example~\ref{example::1b}, it is 92.7--95.3\%. Every cell's binomial 95\%
interval contains the nominal 95\% level. Mean influence-function standard
errors track Monte Carlo standard deviations to within 9\% in Example~1a and
3\% in Example~1b. At $n=2500$ in Example~1a, debiasing reduces the plug-in
bias from $-0.050$ to $0.007$, at the expected cost of additional correction
variance. Across Example~1b, absolute D-IRL bias is at most $0.0054$.
The fitted ratios have mean normalized mass 1.000--1.001, effective-sample
fractions 0.65--0.93, and mean 99th percentiles 12.8--30.7, with no clipping
or numerical failures.

\paragraph{A practical data-fusion normalization.}
The second study adapts Example~\ref{example::norm} to a transition log and an
abundant outcome-summary source. The transition log contains $(S,A,S')$ and
the known logging propensities, but no rewards. The auxiliary source contains
only $(S,Y)$ over the state domain, with action deliberately unavailable and
\[
g(s)=E(Y\mid S=s)=\sum_a\nu(a\mid s)r_0(s,a),
\]
where $\nu$ is a known uniform randomized normalization policy. We generate one
auxiliary sample of size $10^6$ with broad covariate coverage, fit a small smooth
tanh network selected by held-out outcome loss, ensemble three source-only
refits, and freeze the resulting $\widehat g$ across transition-sample
replications. The target discount is $0.8$. Within each of five outer folds, a
polynomial-sieve Gaussian transition model is tuned by held-out next-state loss,
and neural FORE estimates the target occupancy ratio. We report point error and
D-IRL interval coverage at $n\in\{2500,5000,10000\}$. The confidence intervals
condition on the abundant frozen outcome source; joint uncertainty from
estimating $g$ is intentionally negligible by design and is not claimed to be
covered by the present EIF.

\IfFileExists{../../experiments/journal_debiased_irl/artifacts/paper_confirmatory_v2_tables/data_fusion_table.tex}{%
\input{../../experiments/journal_debiased_irl/artifacts/paper_confirmatory_v2_tables/data_fusion_table.tex}%
}{}

All 900 data-fusion replications completed successfully. D-IRL bias ranges
from $-0.0008$ to $0.0029$, and coverage is 93.7--96.0\%; again, each cell's
binomial 95\% interval contains 95\%. The mean standard error agrees closely
with the Monte Carlo standard deviation at $n=2500$ and $5000$ and is 8\%
smaller at $n=10000$, matching that cell's modest undercoverage. FORE retains
unit normalized mass, an effective-sample fraction near 0.67, mean 99th
percentiles between 13.2 and 13.6, and zero clipping or failures. The plug-in
estimator has lower RMSE here because the deliberately abundant outcome source
makes the frozen normalization regression exceptionally accurate; D-IRL adds
the transition-sample correction needed for the reported confidence intervals.
